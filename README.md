...
Before we needed data to test the machine learning algorithm, we first used code from the University of Wisconsin - Madison, with data they gave to train the SVM. 
  ...
  where the bullyingtraceV2.zip has the code necessary to take in lines of text and predict if the text is bullying or not. They include a program enrichment.java that would remove examples that don't contain certain keywords, which we
